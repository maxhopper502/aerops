...
async function savePilotRecord(jobId){
  ...
  // Auto-update actualHours from VDO so billing section reflects pilot record
  const vs=parseFloat(j.completion.vdoStart), ve=parseFloat(j.completion.vdoStop);
  if(vs && ve && ve>vs) j.actualHours=Math.round((ve-vs)*100)/100;
  await saveJob(j);
  renderJobs();
...
async function xeroConnect(){
  ...
  // PKCE flow — no client secret needed or used
  localStorage.setItem('at_xeroClientId',clientId);
  localStorage.removeItem('at_xeroClientSecret');
