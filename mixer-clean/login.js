// Aerotech Mixer PWA — login.js (Firestore REST only, no Firebase SDK)
// Loads mixer PINs from Firestore REST API — no external CDN, no Firebase SDK
// Falls back to hardcoded PINs if Firestore is unreachable
...
(function initMixerLogin(){
  ...
  if(errEl) errEl.textContent = '⏳ Loading mixer...';
...
function doLogin(name){
  ...
  if(typeof window._appLogin === 'function'){
    ...
    var t = setInterval(function(){ if(typeof window._appLogin === 'function'){ clearInterval(t); window._appLogin(name); } }, 100);
