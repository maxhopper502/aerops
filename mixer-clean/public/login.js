// login.js - MINIMAL FIX: XMLHttpRequest + sync on page load
...
function updateBadge(){var b=document.getElementById('sync-badge');if(b){b.textContent='Live';b.className='sync-badge';}}

function fetchJobs(){
  var b=document.getElementById('sync-badge');if(b){b.textContent='Syncing...';}
  var xhr=new XMLHttpRequest();
  xhr.open('GET',FS_BASE+'/jobs?pageSize=200&key='+FS_KEY,true);
  xhr.setRequestHeader('Accept','application/json');
  xhr.timeout=15000;
  xhr.ontimeout=function(){var b=document.getElementById('sync-badge');if(b){b.textContent='Err';}};
  xhr.onload=function(){
    if(xhr.status!==200){var b=document.getElementById('sync-badge');if(b){b.textContent='Err';}return;}
    try{
      var d=JSON.parse(xhr.responseText);
      if(d.error){var b=document.getElementById('sync-badge');if(b){b.textContent='Err';}return;}
      window.jobs=(d.documents||[]).map(fsDoc);
      var b=document.getElementById('sync-badge');if(b){b.textContent='Live';b.className='sync-badge';}
      if(document.getElementById('s-jobs').classList.contains('active'))renderJobs();
    }catch(e){var b=document.getElementById('sync-badge');if(b){b.textContent='Err';}}
  };
  xhr.onerror=function(){var b=document.getElementById('sync-badge');if(b){b.textContent='Err';}};
  xhr.send();
}
...
function showJob(id){
  ...
  w.innerHTML='<button class="back-btn" onclick="showScr(\'s-jobs\')">← Back</button>'+
    ...
    '<div style="margin-top:8px;font-size:14px"><b>Mixer:</b> '+(j.mixerName||'—')+' &nbsp; <b>Pilot:</b> '+(j.pilot||'—')+'</div></div>'+
    '<div style="margin-top:16px"><div class="sec-title">Loads</div>'+
    (loads.length?loads.map(function(l,i){return'<div class="card" style="border-left:4px solid '+(l.complete?'#4caf50':'#ff9800')+'"><div style="font-weight:600">Load '+(i+1)+' — '+(l.chemName||'Unknown')+'</div><div style="font-size:13px;color:#7a8fa6;margin-top:4px">Container: '+(l.containerId||'?')+' &nbsp; Vol: '+(l.volL||0)+'L &nbsp; Rate: '+(l.rate||0)+'L/min</div></div>';}).join(''):'<div style="color:#5a6a7a;padding:20px">No loads</div>')+
    '</div>';
}
...
function selAllJobs(){_loginMixer='ALL';localStorage.setItem('at_mixer','ALL');document.getElementById('pin-section').style.display='none';doLogin('ALL');}
function numTap(k){if(!_loginMixer)return;if(k==='X'){_loginPin=_loginPin.slice(0,-1);drawDots();return;}if(_loginPin.length>=4)return;_loginPin+=k;drawDots();if(_loginPin.length===4){if(MIXER_PINS[_loginMixer]===_loginPin){localStorage.setItem('at_mixer',_loginMixer);doLogin(_loginMixer);}else{document.getElementById('pin-err').textContent='⚠️ Wrong PIN';_loginPin='';drawDots();}}}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  buildMixerGrid();
  fetchJobs(); // <-- THE KEY FIX: start sync on page load
  var saved=localStorage.getItem('at_mixer');
  if(saved==='ALL'){doLogin('ALL');return;}
  if(saved&&MIXER_PINS[saved]){_loginMixer=saved;document.getElementById('pin-section').style.display='block';buildMixerGrid();drawDots();doLogin(saved);}
});
